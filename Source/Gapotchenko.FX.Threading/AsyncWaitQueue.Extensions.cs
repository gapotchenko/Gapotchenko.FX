...
partial struct AsyncWaitQueue<T>
{
    ...
    /// <summary>
    /// Enqueues a cancelable task that will be completed when it is dequeued.
    /// </summary>
    ...
    /// <summary>
    /// Enqueues a cancelable task that will be completed when it is dequeued,
    /// or will fail with <see cref="TimeoutException"/> if the timeout expires before the task is dequeued.
    /// </summary>
    /// <param name="timeout">The timeout.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>A task that will be completed when it is dequeued.</returns>
    public Task<T> Enqueue(TimeSpan timeout, CancellationToken cancellationToken) => Enqueue(null, timeout, cancellationToken);
    ...
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>A task that will be completed when it is dequeued.</returns>
    public Task<T> Enqueue(object? state, TimeSpan timeout, CancellationToken cancellationToken)
    {
        var queue = this;
