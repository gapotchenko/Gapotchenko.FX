...
partial struct ValueInterval<T>
{
    ...
    /// </returns>
    public static bool operator <(in ValueInterval<T> left, T? right) => ComparisonExists(left) && left.CompareTo(right) < 0;
    ...
    /// otherwise, <see langword="false"/>.
    /// </returns>
    public static bool operator <(T? left, in ValueInterval<T> right) => ComparisonExists(right) && right.CompareTo(left) > 0;
    ...
    static bool ComparisonExists(in ValueInterval<T> interval) => !interval.IsEmpty;
}
