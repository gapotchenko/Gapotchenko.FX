...
readonly struct DESManagedTransformKernel : IDisposable
{
    ...
    public void Dispose()
    {
        CryptographicOperations.ZeroMemory(m_WorkingKey);
    }
    ...
    /// </summary>
    readonly int[] m_WorkingKey;

    /// <remarks>
    /// Acknowledgements: James Gillogly, Phil Karn.
    /// </remarks>
    static int[] CreateWorkingKey(ReadOnlySpan<byte> key, bool encrypting)
    {
        ...
        {
            int l = m_PC1[j];
            pc1m[j] = (key[l >>> 3] & m_ByteBit[l & 07]) != 0;
        }
